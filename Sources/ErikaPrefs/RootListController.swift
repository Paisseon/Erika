...
class RootListController: HBRootListController {
	...
	@objc func respring() {
		if FileManager.default.fileExists(atPath: "/Library/MobileSubstrate/DynamicLibraries/shuffle.dylib") {
			HBRespringController.respringAndReturn(to: URL(string: "prefs:root=Tweaks&path=Erika"))
		} else {
			HBRespringController.respringAndReturn(to: URL(string: "prefs:root=Erika"))
		}
	}
}